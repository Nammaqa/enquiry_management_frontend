...
export default function EnrollmentModal({ isOpen, onClose, batchId, batchName, onSuccess }: EnrollmentModalProps) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                const msg = `successfully added ${count} student${count === 1 ? '' : 's'} to batch`;
                
                // Show a clear popup as requested
                window.alert(msg);
    ...
    // Filter students based on search terms and exclude already enrolled
    const filteredStudents = students.filter(student => {
        // Exclude students already enrolled in this specific batch
        const isAlreadyEnrolled = student.enrolledBatches?.some(b => b.id === batchId);
