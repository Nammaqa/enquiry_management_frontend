...
export default function PaidList() {
    ...
    useEffect(() => {
        ...
        const savedSearchTerm = sessionStorage.getItem('paidListSearchTerm');
        const savedSelectedDate = sessionStorage.getItem('paidListSelectedDate');
        const savedCurrentPage = sessionStorage.getItem('paidListCurrentPage');
    ...
    const fetchAllData = async () => {
        ...
        try {
            ...
            // Filter to only fully paid status: those with a generated billing where balance is <= 0
            const fullyPaidEnquiries = enquiriesData.filter(e => {
                if (e.billing && parseFloat(e.billing.balance) <= 0 && parseFloat(e.billing.packageCost) > 0) {
                    return true;
                }
                return false;
            });

            const enquiriesWithPendingLogs = fullyPaidEnquiries.map(enquiry => ({
                ...enquiry,
                callLogs: undefined,
                paymentStatus: 'paid' // override paymentStatus to 'paid' since balance is 0
            }));
            ...
            setError('Failed to load paid list data.');
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        sessionStorage.setItem('paidListSearchTerm', searchTerm);
    }, [searchTerm]);

    // Save selected date to sessionStorage
    useEffect(() => {
        sessionStorage.setItem('paidListSelectedDate', selectedDate);
    }, [selectedDate]);

    // Save current page to sessionStorage
    useEffect(() => {
        sessionStorage.setItem('paidListCurrentPage', currentPage.toString());
    }, [currentPage]);
    ...
    const exportToXLSX = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Paid List');
        ...
        const fileName = `paid_list_${new Date().toISOString().split('T')[0]}.xlsx`;
                            ...
                            {loading && enquiries.length === 0 ? (
                                ...
                                paginatedEnquiries.map((enquiry) => (
                                    <tr key={enquiry.id} className="hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => {
                                        ...
                                        sessionStorage.setItem('paidListSelectedDate', selectedDate);
                                        sessionStorage.setItem('paidListCurrentPage', currentPage.toString());
                                        navigate(`/contact-details/${enquiry.id}?from=paid-list`, { state: { enquiry } });
                                    }}>
